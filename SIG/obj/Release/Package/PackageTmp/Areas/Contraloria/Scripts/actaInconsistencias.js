...
function btnDetallesClick(s, e) {
    ...
    // función para agregar los input al div donde se encuentran los detalles de los recibos, recibe 3 parametros, el primero el el tipo (ej. roto, duplicado, fecha no coincide),
    ...
    function htmlRecibos(tipo, recibo, i) {
        ...
        var html = '<div id="div_' + recibo + '_' + i + '"><fieldset><legend>Recibo ' + tipo + ' nro ' + i + '</legend><table border="0"><tbody><tr><td> \
                    <label>Departamento: </label></td><td><input type="text" id="txtDpto_' + recibo + '_' + i + '"> \
			        </td><td><label>Fecha: </label></td><td><input type="date" id="txtFecha_' + recibo + '_' + i + '"></td></tr><tr><td><label>Municipio </label></td> \
			        <td><input type="text" id="txtMuni_' + recibo + '_' + i + '"></td><td><label>Referencia: </label></td><td> \
                    <input type="text" id="txtRef_' + recibo + '_' + i + '" onchange="informacionRecibo(\'' + recibo + '\',' + i + ',this)"></td></tr><tr><td><label>Aldea: </label></td><td><input type="text" id="txtAld_' + recibo + '_' + i + '"> \
			        </td><td><label>Monto: </label></td><td><input type="text" id="txtMonto_' + recibo + '_' + i + '"></td></tr><tr><td><label>Nombre: </label> \
                    </td><td colspan="3"><input type="text" id="txtNombre_' + recibo + '_' + i + '" size="50"></td></tr></tbody></table></fieldset></div><br>';

        //agrega el html al final del div
        $('#div_' + recibo).append(html);
    }

    // fin código para agregar los controles para los detalles de los recibos
...
function crearActa() {
    ...
    var nombre = txtNombre.GetText(),
        ...
        fchRecepcion = dteRecepcion.GetDate(),
        fchInicio = dteInicio.GetDate(),
        fchFin = dteFin.GetDate();

    //obtener fecha larga
    var meses = new Array("Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre");
    var f = new Date();
    var fecha = f.getDate() + " días del mes de " + meses[f.getMonth()] + " de " + f.getFullYear();
    ...
    pdf.text(1.7, 1, 'ACTA DE INCONCISTENCIAS BONO VIDA MEJOR');

    // para crear la fecha que va en el titulo del acta
    var fchTitulo, strFchTitulo;
    if (fchInicio.getDate() == fchFin.getDate() && fchInicio.getMonth() == fchFin.getMonth() && fchInicio.getFullYear() == fchFin.getFullYear()) {
        fchTitulo = fchInicio.getDate() + ' DE ' + meses[fchInicio.getMonth()].toUpperCase() + ' DEL AÑO ' + fchInicio.getFullYear();
        //strFchTitulo = 'PAGO ' + dpto + ' ' + fchTitulo;
        pdf.text(1.73, 1.3, 'PAGO ' + dpto + ' ' + fchTitulo);
    } else if (fchInicio.getMonth() == fchFin.getMonth() && fchInicio.getFullYear() == fchFin.getFullYear()) {
        fchTitulo = fchInicio.getDate() + ' AL ' + fchFin.getDate() + ' DE ' + meses[fchInicio.getMonth()].toUpperCase() + ' DEL AÑO ' + fchInicio.getFullYear();
        //strFchTitulo = 'PAGO ' + dpto + ' ' + fchTitulo;
        pdf.text(1.18, 1.3, 'PAGO ' + dpto + ' DEL ' + fchTitulo);
    } else if (fchInicio.getFullYear() == fchFin.getFullYear()) {
        fchTitulo = fchInicio.getDate() + ' DE ' + meses[fchInicio.getMonth()].toUpperCase() + ' AL ' + fchFin.getDate() + ' DE ' + meses[fchFin.getMonth()].toUpperCase() + ' DEL AÑO ' + fchInicio.getFullYear();
        //strFchTitulo = 'PAGO ' + dpto + ' ' + fchTitulo;
        pdf.text(0.7, 1.3, 'PAGO ' + dpto + ' DEL ' + fchTitulo);
    } else {
        fchTitulo = fchInicio.getDate() + ' DE ' + meses[fchInicio.getMonth()].toUpperCase() + ' DEL AÑO ' + fchInicio.getFullYear() + ' AL ' + fchFin.getDate() + ' DE ' + meses[fchFin.getMonth()].toUpperCase() + ' DEL AÑO ' + fchFin.getFullYear();
    }
    ...
    //loremipsum += 'de Representante de Contraloría y Cierre y ' + nombre + ', en su condición de contralor Bono Vida Mejor. A continuación se adjunta las inconsistencias ';
    //loremipsum += 'encontradas en el pago del Departamentos de ' + dpto + ' recibida el ' + fchRecepcion.getDate() + " de " + meses[fchRecepcion.getMonth()] + " de ";
    //loremipsum += fchRecepcion.getFullYear() + '.\n\nLo que se encontró:\n';

    loremipsum = 'En la Ciudad de Tegucigalpa, Municipio del Distrito Central a los ' + fecha + '. Reunidos en las oficinas de la Secretaía de ';
    loremipsum += 'Desarrollo e Inclusión Social en el Departamento de Contraloría y Cierre, las  siguientes personas en su orden y puesto: ' + usuario + ' en su condición ';
    loremipsum += 'de Representante de Contraloría y Cierre y ' + nombre + ', en su condición de contralor Bono Vida Mejor. A continuación se adjunta las inconsistencias encontradas ';

    //debugger;
    if (dpto != "") {
        loremipsum += 'en el pago del Departamentos de ' + dpto + ' ';
    }
    
    loremipsum += 'recibida el ' + fchRecepcion.getDate() + " de " + meses[fchRecepcion.getMonth()] + " de ";
    loremipsum += fchRecepcion.getFullYear() + '.\n\nLo que se encontró:\n';
    ...
    if (dpto != "") {
        strFchTitulo = 'PAGOS DE ' + dpto + ' ' + fchTitulo;
    } else {
        strFchTitulo = 'PAGOS DE ' + fchTitulo;
    }
    ...
    var dd = {
        ...
        content: [
            {
                text: 'ACTA DE INCONCISTENCIAS BONO VIDA MEJOR\n' + strFchTitulo + '\n\n',
                style: 'header',
                alignment: 'center'
            },
            contenidoEncabezado
    ...
    if (flagRecibos) {
        ...
        if (countRecibos[0] > 0) {
            agregarRecibo(countRecibos[0], 'sin huella', 'RSH', 0);
        }
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // inicio de funciones para agregar el texto al acta


    //fución que construye el texto para el recibo al acta
    function agregarRecibo(numRecibos, tipo, recibo, num) {

        loremipsum = '\n* ' + countRecibos[num] + ' Recibos ' + tipo + '.';
        addText(loremipsum, 7, 1, verticalOffset);
        //dd.content.push(loremipsum);
        ...
        for (i = 1; i <= numRecibos; i++) {
            loremipsum = '';
            loremipsum += '- Recibo ' + tipo + ' nro ' + i + ': Depto, ' + $('#txtDpto_' + recibo + '_' + i).val() + ', Municipio de ' + $('#txtMuni_' + recibo + '_' + i).val() + ', Aldea ';
            loremipsum += $('#txtAld_' + recibo + '_' + i).val() + ', Fecha: ' + $('#txtFecha_' + recibo + '_' + i).val() + ', Ref: ' + $('#txtRef_' + recibo + '_' + i).val() + ', Nombre: ' + $('#txtNombre_' + recibo + '_' + i).val();
            loremipsum += ', monto: ' + $('#txtMonto_' + recibo + '_' + i).val() + '.';
    ...
    function agregarActa(numActa, tipo, acta, num, countMovil, countAgencia) {
        ...
        if (tipo == "formato incorrecto" || tipo == "fecha incorrecta" || tipo == "fondo incorrecto") {
            ...
            if (tipo == "formato incorrecto") {
                ...
                loremipsum = '\nLas Actas de Cierre presentan una diferencia en el formato de las columnas del cuadro resumen del pago, debiendo ser lo correcto la columna de lo programado primera, luego ';
            ...
            if (tipo == "fecha incorrecta") {
                ...
                loremipsum = '\nLos siguientes Departamentos presentan las Actas de Cierre con fecha incorrecta:';
                addText(loremipsum, 7, 1, verticalOffset);
            ...
            if (tipo == "fondo incorrecto") {
                ...
                loremipsum = '\nLos siguientes Departamentos presentan en el Acta de Cierre el Fondo Incorrecto:';
                addText(loremipsum, 7, 1, verticalOffset);
    ...
    function agregarReporte(numReportes, tipo, reporte, num) {
        ...
        loremipsum = '\n* ' + countReportes[num] + ' Reporte ' + tipo + '.';
        addText(loremipsum, 7, 1, verticalOffset);
    ...
    function agregarNotaCargo(numNotas, tipo, nota, num) {
        //debugger;
        loremipsum = '\n* ' + countNotas[num] + ' Notas de Cargo ' + tipo + '.';
        addText(loremipsum, 7, 1, verticalOffset);
    ...
    // función que agrega el texto al acta, recibe el párrafo, el ancho del del cual se desea que sea el parrafo
    // la pasición X y Y en las cuales empezar a escribir el texto
    // NOTA: no mandar a escribir párrafos muy largos ocurre en problema con los texto largos que no pude resolver
...
// socilita al server la información que deberia de traer el recibo, recibe el tipo de recibo, el input que llamo
// a la función del cual se extre la el valor  el cual debe de ser de la siguiente manera 775216-2 
// siendo el primer valor el número de página y el segundo después del guión el número de referencia, 
// el valor de i es el numero de recibo de ese tipo en el acta
function informacionRecibo(recibo, i, input) {

    var value = input.value;
    var referencia = value.split('-')

    if (referencia.length == 2) {
        $.ajax({
            url: '/Contraloria/Actas/infoRecibo',
            dataType: 'Json',
            data: {
                pagina: referencia[0],
                registro: referencia[1]
            },
            success: function (response) {

                if (response) {
                    
                    $('#txtDpto_' + recibo + '_' + i).val(response.dpto);
                    $('#txtMuni_' + recibo + '_' + i).val(response.muni);
                    $('#txtAld_' + recibo + '_' + i).val(response.aldea);
                    $('#txtFecha_' + recibo + '_' + i).val(response.fecha);
                    $('#txtMonto_' + recibo + '_' + i).val(response.monto);
                    $('#txtNombre_' + recibo + '_' + i).val(response.nombre);
        ...
        alert('Error en el formato de la referencia./nEl formato debe de ser XXXXXX-X');
    }
}
